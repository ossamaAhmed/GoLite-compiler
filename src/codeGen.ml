...
let init_struct_type field_dcl_list struct_iden = 
    ...
    let print_struct_field_type type_name identifier = match identifier, type_name with
        ...
        | Identifier(field_iden, _), Primitivetype(value, _) ->
            ...
            | "rune" -> println_struct_string (".field "^field_iden^" I")
            | "bool" -> println_struct_string (".field "^field_iden^" I")
                    ...
                    (match symt2 with 
                        | SymType(SymStruct(field_list)) -> let field_sym_type = find_field iden field_list in
                            (match field_sym_type with
                                ...
                                | SymBool -> println_one_tab ("getfield "^struct_class^"/"^iden^" "^"I")
                                | SymArray(sym_type) -> ()
                                | SymSlice(sym_type) -> ()
                                | SymStruct(fields) -> ()
                                | NotDefined -> ()
...
(*                                         println_one_tab (generate_load (SymStruct(field_list)) struct_var_name linenum2); *)
                                ...
                                | SymBool -> println_one_tab ("putfield "^struct_class^"/"^iden^" "^"I")
                                | SymArray(sym_type) -> ()
                                | SymSlice(sym_type) -> ()
                                | SymStruct(fields) -> ()
                                | NotDefined -> ()
        ...
        | Print(exprs, _) -> 
            let print_jasmin_exp exp = 
                ...
                match (print_expr exp) with 
                    ...
                    | SymBool -> println_one_tab "invokevirtual java/io/PrintStream/print(I)V";
                    | NotDefined -> ()
            in
        ...
        | Println(exprs, _) -> 
            let println_jasmin_exp exp = 
                ...
                match (print_expr exp) with 
                    ...
                    | SymBool -> println_one_tab "invokevirtual java/io/PrintStream/println(I)V";
                    | NotDefined -> ()
            in
