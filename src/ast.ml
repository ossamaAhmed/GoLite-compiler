...
let get_exp_type exp = match exp with 
    | OperandName(value, linenum, symt) -> "OperandName"
    | AndAndOp(exp1, exp2, _, _) -> "AndAndOp"
    | OrOrOp(exp1, exp2, _, _) -> "OrOrOp"
    | EqualEqualCmp(exp1, exp2, _, _) -> "EqualEqualCmp"
    | NotEqualCmp(exp1, exp2, _, _) -> "NotEqualCmp"
    | LessThanCmp(exp1, exp2, _, _) -> "LessThanCmp"
    | GreaterThanCmp (exp1, exp2, _, _) -> "GreaterThanCmp"
    | LessThanOrEqualCmp(exp1, exp2, _, _) -> "LessThanOrEqualCmp"
    | GreaterThanOrEqualCmp(exp1, exp2, _, _) -> "GreaterThanOrEqualCmp"
    | AddOp(exp1, exp2, _, _) -> "AddOp"
    | MinusOp(exp1, exp2, _, _) -> "MinusOp"
    | OrOp (exp1, exp2, _, _) -> "OrOp"
    ...
    | UnaryCaret(exp1, _, _) -> ""
    | Value(value, _, _) -> ""
    | Selectorexpr(exp1, Identifier(iden, _), linenum, symbolType) -> "Selectorexpr"
    | TypeCastExpr(typename, exp1, _, symbolType) -> "TypeCastExpr"
    | Appendexpr(Identifier(iden, _),exp1, _, _) -> "Appendexpr"
